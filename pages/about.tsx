...
export default function About() {
  // Assume 'kn' for Kannada and 'en' for English; replace this with actual language state from the Home page
  const [language, setLanguage] = useState('kn'); // Default to Kannada for now
...
"It is said that the current stone idol, in the form of a Linga, was brought and installed around 800 years ago by the Lingayat kings from North Karnataka. After the decline of the Ballala dynasty, the temple came under the rule of the Kodagu kings. During the state administration period, the temple was granted significant agricultural land, which is an important part of its historical legacy.."
    ...
    kn: [
      "ಶ್ರೀ ಹರಿಹರೇಶ್ವರವು ಅನಾದಿ ಕಾಲದಿಂದಲೂ ಇದ್ದು ಋಷಿ ಶ್ರೇಷ್ಠರು ಈ ತೀರ್ಥಕ್ಷೇತ್ರದಲ್ಲಿ ತಪಸ್ಸನ್ನಾಚರಿಸಿ ಹರಿ ಮತ್ತು ಹರ ಇಬ್ಬರನ್ನು ಒಂದೇ ಮೂರ್ತಿಯಲ್ಲಿ ಸಂಕಲ್ಪಿಸಿ ಸ್ಥಾಪಿಸಿದ ಕ್ಷೇತ್ರವೇ ಶ್ರೀ ಹರಿಹರೇಶ್ವರ ದೇವಾಲಯ.",
      ...
      "ಸಿದ್ದ ಪರ್ವತದಿಂದ ಹರಿದು ಬರುವ ನದಿಯು 'ಕೋಟಿ ತೀರ್ಥ'ವೆಂದು ಪ್ರಸಿದ್ದಿಯಾಗಿದೆ. ಈ ನದಿಯು ಸಂಗಮವಾಗುವ ಹರಿಹರೇಶ್ವರ ನೆಲೆಯಲ್ಲಿ ಪುಣ್ಯಸ್ನಾನ ಮಾಡಿ ಹರಿಹರೇಶ್ವರನ ದರುಶನ ಪಡೆಯುವುದರಿಂದ ಔಷಧಿ ರಹಿತವಾಗಿ ಕಾಯಿಲೆಗಳು, ಮನೋರೋಗ, ಚರ್ಮರೋಗಾಧಿಗಳು ವಾಸಿಯಾಗುತ್ತವೆ ಎಂಬ ನಂಬಿಕೆ ಇದೆ.",
      ...
      "ಪ್ರಸ್ತುತ ಇರುವ ಶಿಲಾಮೂರ್ತಿಯು ಲಿಂಗಸ್ವರೂಪಿಯಾಗಿದ್ದು ಸುಮಾರು 800 ವರ್ಷಗಳ ಹಿಂದೆ ಉತ್ತರ ಕರ್ನಾಟಕದಿಂದ ಲಿಂಗಾಯತ ಅರಸರು ತಂದು ಸ್ಥಾಪಿಸಿದರು ಎಂದು ಪ್ರತೀತಿ."
    ],
  };

  return (
    <main className="p-8 max-w-3xl mx-auto text-black-800" style={{ color:'#333',backgroundColor: 'transparent',fontWeight: 'bold'  }}>
      {/* History heading */}
      <h1 className="text-5xl font-bold text-center text-orange-700 mb-6">
  {language === 'en' ? 'History' : 'ಇತಿಹಾಸ'}
      ...
      {/* Placeholder language switcher button (for testing purposes) */}
      <div className="mt-6 flex justify-center">
        <button 
          onClick={() => setLanguage(language === 'en' ? 'kn' : 'en')} 
          className="px-4 py-2 bg-orange-700 text-white rounded-lg hover:bg-orange-600 transition"
        >
